#include "code_objects/neuron_1_spike_thresholder_codeobject.h"
#include "objects.h"
...
const size_t _numv = 1;
    ...
    for(size_t _idx=0; _idx<N; _idx++)
    {
        ...
        const double v = _ptr_array_neuron_1_v[_idx];
        const char _cond = v > _lio_1;

        if(_cond) {
            _ptr_array_neuron_1__spikespace[_count++] = _idx;
        }
    }
    _ptr_array_neuron_1__spikespace[N] = _count;
